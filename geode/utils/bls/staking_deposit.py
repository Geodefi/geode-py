...
def from_mnemonic(
    ...
    for index in key_indices:
        ...
        for amount in [DEPOSIT_SIZE.PROPOSAL.value, DEPOSIT_SIZE.STAKE.value]:
            ...
            datum_dict.update({"fork_version": GENESIS_FORK_VERSION[chain_id]})
        ...
        keystore_data.append(
            signing_keystore(signing_sk, signing_key_path, mnemonic_password)
        )
...
def _get_word_list(path: str) -> List[str]:
    ...
    """
    dirty_list = open(
        os.path.join(path, "word_list.txt"), encoding="utf-8"
    ).readlines()
...
def get_seed(*, mnemonic: str, password: str) -> bytes:
    ...
    return PBKDF2(
        password=encoded_mnemonic, salt=salt, dklen=64, c=2048, prf="sha512"
    )
...
def signing_keystore(
    signing_sk: int, signing_key_path: str, password: str
) -> Keystore:
    secret = signing_sk.to_bytes(32, "big")
    return asdict(
        ScryptKeystore.encrypt(
            secret=secret, password=password, path=signing_key_path
        )
    )
